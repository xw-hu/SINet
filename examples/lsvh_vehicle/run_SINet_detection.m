...
binary_file = [root_dir 'SINet_lsvh_train_2nd_iter_75000.caffemodel'];
...
% set LSVH dataset directory
root_dir = '/home/xwhu/LSVH/';
image_dir = [root_dir 'images/'];
comp_id='SINet_LSVH_result'; 

dataType = 'test';
image_name = importdata([root_dir dataType '.txt']);

nImg=length(image_name);

% choose the right input size
 imgW = 1344; imgH = 768;

mu = ones(1,1,3); mu(:,:,1:3) = [118 121 120];
...
cls_ids = [2 3 4]; num_cls=length(cls_ids); 
obj_names = {'bg','car','bus','van'};
...
for k = 1 : nImg
  ...
  img_path = [image_dir image_name{k} '.jpg'];
  ...
  tmp_cls1=squeeze(outputs{3});
  tmp_cls2=squeeze(outputs{4});
  
  hash_table = net.blobs('hash_table').get_data();
  roi_num = size(hash_table,4);
  
  bbox_preds = zeros(roi_num,16);
  cls_pred = zeros(roi_num,4);
  
  for i=1:roi_num
      if (hash_table(1,1,2,i)==2) %large
  ...
  tmp=squeeze(outputs{5}); tmp = tmp'; tmp = tmp(:,2:end); 
  ...
  for i = 1 : num_cls
    ...
    bbox_pred = bbox_pred.*repmat(bbox_stds,[size(bbox_pred,1) 1]);  %bbox_stds(size(bbox_pred,1), 1)    bbox_pred = bbox_pred+repmat(bbox_means,[size(bbox_pred,1) 1]);  %bbox_means(size(bbox_pred,1), 1)
...
%       end 
      ...
      for j = 1:size(bbs_show,1)
        rectangle('Position',bbs_show(j,1:4),'EdgeColor','r','LineWidth',1);
        show_text = sprintf('%s',obj_names{id});
        %show_text = sprintf('%s=%.2f',obj_names{id},bbs_show(j,5));
        x = bbs_show(j,1)+0.5*bbs_show(j,3);
        text(x,bbs_show(j,2),show_text,'color','r','HorizontalAlignment',...
            'center', 'VerticalAlignment','bottom','FontWeight','bold', 'FontSize',8);
      end 
      
      %% if uncomment the following three lines, the results can be saved in 'results/'
      handle=gca;%gcf
      saveas(handle,['results/'  image_name{k} '.png']);
      clear handle;
    end
  end
...
for i=1:size(final_detect_boxes,1)
    for j=1:num_cls
        if (isempty(final_detect_boxes{i,j}))
            final_detect_boxes{i,j} = [i,0,0,0,0,0];
        end
    end
end
