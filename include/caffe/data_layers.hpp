...
template <typename Dtype>
class Image2GtDataLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit Image2GtDataLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~Image2GtDataLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "Image2GtData"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
 ...
 protected:
  ...
  shared_ptr<Caffe::RNG> prefetch_rng_;
  vector<std::pair<std::string, vector<int> > > image_database_;
  enum Image2GtField { X1, Y1, X2, Y2, LABELSUB, LABEL, IGNORE, NUM };
  vector<vector<vector<float> > > windows_;
  vector<vector<vector<float> > > roni_windows_;
  vector<Dtype> mean_values_;
  bool has_mean_values_;
