...
const int32_t MIN_HEIGHT[4] = {39, 39, 39, 39};     // minimum height for evaluated groundtruth/detections
const int32_t MAX_HEIGHT[4] = {66, 66, 66, 66};     // maximum height for evaluated groundtruth/detections
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, SCENE scene){
  ...
  for(int32_t i=0;i<gt.size(); i++){
    ...
    bool ignore = false;
    if(height<MIN_HEIGHT[scene] || gt[i].scene!=scene ||height>MAX_HEIGHT[scene])
    	ignore = true;
...
bool eval(string gt_dir, string result_dir, string data_list_path){
  ...
  for (int32_t i=0; i<N_VALIMAGES; i++) {
	...
	int scene=-1;
	if (scenetype == "Sunny") scene = 0;
	if (scenetype == "Rainy") scene = 0;
	if (scenetype == "Blurry") scene = 3;
	if (scenetype == "Occuluded") scene = 3;
