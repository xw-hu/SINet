...
void LiftedStructSimilaritySoftmaxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  //cout<<"0 num: "<<bottom[0]->num()<<endl;
  //cout<<"0 channels: "<<bottom[0]->channels()<<endl;
  //cout<<"0 height: "<<bottom[0]->height()<<endl;
  //cout<<"0 width: "<<bottom[0]->width()<<endl;

  //cout<<"1 num: "<<bottom[1]->num()<<endl;
  //cout<<"1 channels: "<<bottom[1]->channels()<<endl;
  //cout<<"1 height: "<<bottom[1]->height()<<endl;
  //cout<<"1 width: "<<bottom[1]->width()<<endl;
...
void LiftedStructSimilaritySoftmaxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int i=0; i<N_; i++){
    for (int j=0; j<N_; j++){
      label_mat[i][j] = (bottom[1]->cpu_data()[i] == bottom[1]->cpu_data()[j]); //same class ?
    }
  }
  ...
  for (int i=0; i<N_; i++){
    for (int j=i+1; j<N_; j++){
      ...
      //cout<<"i,j: "<<i<<" "<<j<<" label: "<< _mat[i][j]<<endl;
      ...
      if (label_mat[i][j]){
        ...
        if(num_negatives==0)  //HU XIAOWEI 2016.9
        {
            //cout<<"num_negatives: "<<num_negatives<<endl;
            break; 
        }
  ...
  if (num_constraints != Dtype(0.0)) //HU XIAOWEI 2016.10
  {
...
void LiftedStructSimilaritySoftmaxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  Dtype alpha = Dtype(1.0);  //HU Xiaowei 2016.10
  
  if (num_constraints != Dtype(0.0))
  {
      alpha = top[0]->cpu_diff()[0] / num_constraints / Dtype(2.0);
  }
