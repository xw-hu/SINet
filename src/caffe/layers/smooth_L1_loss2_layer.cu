...
__global__ void SmoothL12Forward(const int n, const Dtype* in, Dtype* out,
    Dtype sigma2) {
...
template <typename Dtype>
void SmoothL1Loss2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  
  if ( static_cast<int>((bottom[4]->cpu_data())[branch_id_-1])==0 )
  {
    top[0]->mutable_cpu_data()[0] = 0;

    for(int k=0; k++; k<diff_.count())
    {
       diff_.mutable_gpu_data()[k] = Dtype(0.0);
    }
  ...
  SmoothL12Forward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
template <typename Dtype>
__global__ void SmoothL12Backward(const int n, const Dtype* in, Dtype* out,
    Dtype sigma2) {
...
template <typename Dtype>
void SmoothL1Loss2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {

  // after forwards, diff_ holds w_in * (b0 - b1)
  int count = diff_.count();
  SmoothL12Backward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
INSTANTIATE_LAYER_GPU_FUNCS(SmoothL1Loss2Layer);
