...
// ------------------------------------------------------------------
// Modify by Xiaowei Hu to provide two kinds of ground truth
...
Image2GtDataLayer<Dtype>::~Image2GtDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void Image2GtDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  //    width
  //    num_gts
  //    label labelsub ignore x1 y1 x2 y2  add sub_label
  //    num_roni
  //    x1 y1 x2 y2

  LOG(INFO) << "Window data layer:" << std::endl
      << "  batch size: "
      << this->layer_param_.image_2gt_data_param().batch_size() << std::endl
      << "  cache_images: "
      << this->layer_param_.image_2gt_data_param().cache_images() << std::endl
      << "  root_folder: "
      << this->layer_param_.image_2gt_data_param().root_folder();

  cache_images_ = this->layer_param_.image_2gt_data_param().cache_images();
  string root_folder = this->layer_param_.image_2gt_data_param().root_folder();

  // reset the random generator seed
  const unsigned int prefetch_rng_seed = caffe_rng_rand();
  prefetch_rng_.reset(new Caffe::RNG(prefetch_rng_seed));

  const int fg_img_multiple = this->layer_param_.image_2gt_data_param().fg_img_multiple();
  int list_index = 0;

  // source
  std::ifstream infile(this->layer_param_.image_2gt_data_param().source().c_str());
  CHECK(infile.good()) << "Failed to open window file "
      << this->layer_param_.image_2gt_data_param().source() << std::endl;

  map<int, int> label_hist;
  label_hist.insert(std::make_pair(0, 0));

  map<int, int> labelsub_hist;  // HU Xiaowei  subclass for label
  labelsub_hist.insert(std::make_pair(0, 0));
  ...
  do {
    ...
    for (int i = 0; i < num_windows; ++i) {
      int label, labelsub, ignore, x1, y1, x2, y2;  //HU Xiaowei
      infile >> label >> labelsub >> ignore >> x1 >> y1 >> x2 >> y2;

      vector<float> window(Image2GtDataLayer::NUM);
      window[Image2GtDataLayer::LABELSUB] = labelsub;
      window[Image2GtDataLayer::LABEL] = label;
      window[Image2GtDataLayer::IGNORE] = ignore;
      window[Image2GtDataLayer::X1] = x1;
      window[Image2GtDataLayer::Y1] = y1;
      window[Image2GtDataLayer::X2] = x2;
      window[Image2GtDataLayer::Y2] = y2;
      ...
      // add window to list
      label = window[Image2GtDataLayer::LABEL];
      labelsub = window[Image2GtDataLayer::LABELSUB];
      CHECK_GT(label, 0);
	  CHECK_GT(labelsub, 0);
      windows.push_back(window);
      label_hist.insert(std::make_pair(label, 0));
      label_hist[label]++;
	  labelsub_hist.insert(std::make_pair(labelsub, 0));  // just used for print out
      labelsub_hist[labelsub]++;
    }
    ...
    for (int i = 0; i < num_roni_windows; ++i) {
      ...
      vector<float> roni_window(Image2GtDataLayer::NUM-3);  //3 dont have 
      roni_window[Image2GtDataLayer::X1] = x1;
      roni_window[Image2GtDataLayer::Y1] = y1;
      roni_window[Image2GtDataLayer::X2] = x2;
      roni_window[Image2GtDataLayer::Y2] = y2;
  ...
  for (map<int, int>::iterator it = label_hist.begin();
   ...
   for (map<int, int>::iterator it = labelsub_hist.begin();
      it != labelsub_hist.end(); ++it) {
    LOG(INFO) << "subclass " << it->first << " has " << labelsub_hist[it->first]
              << " samples";
  }
  
  if (this->layer_param_.image_2gt_data_param().shuffle()) {
  ...
  template_width = img_width; template_height = img_height;  // image original size or resize the image
  if (this->layer_param_.image_2gt_data_param().has_resize_width()
      && this->layer_param_.image_2gt_data_param().has_resize_height()) {
    template_width = this->layer_param_.image_2gt_data_param().resize_width();
    template_height = this->layer_param_.image_2gt_data_param().resize_height();
  }
  if (this->layer_param_.image_2gt_data_param().has_crop_width()  //crop the image from the resized image
      && this->layer_param_.image_2gt_data_param().has_crop_height()) {
    int crop_width = this->layer_param_.image_2gt_data_param().crop_width();
    int crop_height = this->layer_param_.image_2gt_data_param().crop_height();
    if (crop_width > 0) {
    ...
    if (crop_height > 0) {
      ...
      template_height = crop_height;   //final size 
    }
  }

  // image reshape
  const int batch_size = this->layer_param_.image_2gt_data_param().batch_size();
  top[0]->Reshape(batch_size, channels, template_height, template_width);  //shape for top[0] 
  for (int i = 0; i < this->PREFETCH_COUNT; ++i)  //3
    this->prefetch_[i].data_.Reshape(batch_size, channels, template_height, template_width);

  LOG(INFO) << "output data size: " << top[0]->num() << ","
      << top[0]->channels() << "," << top[0]->height() << ","
      << top[0]->width();
  // label reshape (on channel axis, [label x y w h iou])  //one label
  label_channel_ = 2+this->layer_param_.image_2gt_data_param().coord_num();
  label_blob_num_ = this->layer_param_.image_2gt_data_param().downsample_rate_size(); //num of downsmaple_rate

  ms_sub_label_ = this->layer_param_.image_2gt_data_param().ms_sub_lable(); //HU sublabel for 5x5 7x7
  //CHECK_GE(label_blob_num_, 1); //modify by HUXIAOWEI
  CHECK_GE(label_blob_num_, 0);
  CHECK_EQ(label_blob_num_, this->layer_param_.image_2gt_data_param().field_h_size());
  CHECK_EQ(label_blob_num_, this->layer_param_.image_2gt_data_param().field_w_size());
  for (int nn = 0; nn < label_blob_num_; nn++) { 
    downsample_rates_.push_back(this->layer_param_.image_2gt_data_param().downsample_rate(nn));
    field_ws_.push_back(this->layer_param_.image_2gt_data_param().field_w(nn));
    field_hs_.push_back(this->layer_param_.image_2gt_data_param().field_h(nn));
  ...
  output_gt_boxes_ = this->layer_param_.image_2gt_data_param().output_gt_boxes();  //here is 2 gt boxes
  output_2gt_boxes_ = this->layer_param_.image_2gt_data_param().output_2gt_boxes();  //here is 2 gt boxes  must be 2 gt boxes
  if (output_gt_boxes_) {
    //dummy reshape
    top[label_blob_num_+1]->Reshape(1, 7, 1, 1);    // 7 label x1 y1 x2 y2....
    for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
      shared_ptr<Blob<Dtype> > label_blob_pointer(new Blob<Dtype>());
  ...
  /////////////////////////////////////////////////////////////////////////second label
    if (output_gt_boxes_ && output_2gt_boxes_) {
    //dummy reshape
    top[label_blob_num_+2]->Reshape(1, 7, 1, 1);    // 7 labelsub x1 y1 x2 y2.... just for shape
    for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
      shared_ptr<Blob<Dtype> > label_blob_pointer(new Blob<Dtype>());
      label_blob_pointer->Reshape(1, 7, 1, 1);
      this->prefetch_[i].labels_.push_back(label_blob_pointer);
    }
    LOG(INFO) << "output gt boxes size: " << top[label_blob_num_+2]->num() << ","
        << top[label_blob_num_+2]->channels() << "," << top[label_blob_num_+2]->height() 
        << "," << top[label_blob_num_+2]->width();
  }
...
template <typename Dtype>
unsigned int Image2GtDataLayer<Dtype>::PrefetchRand() {  //random value
  CHECK(prefetch_rng_);
...
template <typename Dtype>
void Image2GtDataLayer<Dtype>::ShuffleList() {
  caffe::rng_t* prefetch_rng =
      static_cast<caffe::rng_t*>(prefetch_rng_->generator());
...
template <typename Dtype>
void Image2GtDataLayer<Dtype>::BoundingboxAffine(vector<vector<float> >& bbs,
        float w_scale, float h_scale, float w_shift, float h_shift) {
  for (int ww = 0; ww < bbs.size(); ww++) {
    // rescale
    bbs[ww][Image2GtDataLayer<Dtype>::X1] = w_scale*bbs[ww][Image2GtDataLayer<Dtype>::X1];
    bbs[ww][Image2GtDataLayer<Dtype>::X2] = w_scale*bbs[ww][Image2GtDataLayer<Dtype>::X2];
    bbs[ww][Image2GtDataLayer<Dtype>::Y1] = h_scale*bbs[ww][Image2GtDataLayer<Dtype>::Y1];
    bbs[ww][Image2GtDataLayer<Dtype>::Y2] = h_scale*bbs[ww][Image2GtDataLayer<Dtype>::Y2];
    // shift
    bbs[ww][Image2GtDataLayer<Dtype>::X1] = bbs[ww][Image2GtDataLayer<Dtype>::X1]+w_shift;
    bbs[ww][Image2GtDataLayer<Dtype>::X2] = bbs[ww][Image2GtDataLayer<Dtype>::X2]+w_shift;
    bbs[ww][Image2GtDataLayer<Dtype>::Y1] = bbs[ww][Image2GtDataLayer<Dtype>::Y1]+h_shift;
    bbs[ww][Image2GtDataLayer<Dtype>::Y2] = bbs[ww][Image2GtDataLayer<Dtype>::Y2]+h_shift;
  }
}

// Thread fetching the data
template <typename Dtype>
void Image2GtDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  const Dtype scale = this->layer_param_.image_2gt_data_param().scale();
  const int batch_size = this->layer_param_.image_2gt_data_param().batch_size();
  const bool mirror = this->transform_param_.mirror();
  const float fg_threshold = this->layer_param_.image_2gt_data_param().fg_threshold();
  const float min_gt_width = this->layer_param_.image_2gt_data_param().min_gt_width();
  const float min_gt_height = this->layer_param_.image_2gt_data_param().min_gt_height();
  const bool do_multiple_scale = this->layer_param_.image_2gt_data_param().do_multiple_scale()
                                 && this->layer_param_.image_2gt_data_param().has_min_scale()
                                 && this->layer_param_.image_2gt_data_param().has_max_scale();
  const bool whaspect = this->layer_param_.image_2gt_data_param().has_min_whaspect()
                        && this->layer_param_.image_2gt_data_param().has_max_whaspect();
  ...
  for (int dummy = 0; dummy < batch_size; ++dummy) {
      ...
      if (do_mirror) {
        cv::flip(cv_img, cv_img, 1); //just flip without increase the number of images
        for (int ww = 0; ww < windows.size(); ++ww) {
          float tmp;
          windows[ww][Image2GtDataLayer<Dtype>::X1] = img_width-windows[ww][Image2GtDataLayer<Dtype>::X1];
          windows[ww][Image2GtDataLayer<Dtype>::X2] = img_width-windows[ww][Image2GtDataLayer<Dtype>::X2];
          tmp = windows[ww][Image2GtDataLayer<Dtype>::X1];
          windows[ww][Image2GtDataLayer<Dtype>::X1] = windows[ww][Image2GtDataLayer<Dtype>::X2];
          windows[ww][Image2GtDataLayer<Dtype>::X2] = tmp;
        }
        
        for (int ww = 0; ww < roni_windows.size(); ++ww) {
          float tmp;
          roni_windows[ww][Image2GtDataLayer<Dtype>::X1] = img_width-roni_windows[ww][Image2GtDataLayer<Dtype>::X1];
          roni_windows[ww][Image2GtDataLayer<Dtype>::X2] = img_width-roni_windows[ww][Image2GtDataLayer<Dtype>::X2];
          tmp = roni_windows[ww][Image2GtDataLayer<Dtype>::X1];
          roni_windows[ww][Image2GtDataLayer<Dtype>::X1] = roni_windows[ww][Image2GtDataLayer<Dtype>::X2];
          roni_windows[ww][Image2GtDataLayer<Dtype>::X2] = tmp;
        }
      }
      // resize image if needed
      if (this->layer_param_.image_2gt_data_param().has_resize_width()
          && this->layer_param_.image_2gt_data_param().has_resize_height()) {
        int resize_width = this->layer_param_.image_2gt_data_param().resize_width();
        int resize_height = this->layer_param_.image_2gt_data_param().resize_height();
      ...
      if (windows.size() != 0) {
        ...
        sel_center_x = (windows[sel_id][Image2GtDataLayer<Dtype>::X1]+windows[sel_id][Image2GtDataLayer<Dtype>::X2])/2.0;
        sel_center_y = (windows[sel_id][Image2GtDataLayer<Dtype>::Y1]+windows[sel_id][Image2GtDataLayer<Dtype>::Y2])/2.0;
      } else {
      ...
      if (do_multiple_scale && windows.size()!=0 && PrefetchRand()%2) {
        float octave = 2; 
        int min_scale = this->layer_param_.image_2gt_data_param().min_scale();
        int max_scale = this->layer_param_.image_2gt_data_param().max_scale();
        float scale_order = log2(float(max_scale)/float(min_scale));  //resize 7
        ...
        float X1 = windows[sel_id][Image2GtDataLayer<Dtype>::X1];
        float X2 = windows[sel_id][Image2GtDataLayer<Dtype>::X2];
        float Y1 = windows[sel_id][Image2GtDataLayer<Dtype>::Y1];
        float Y2 = windows[sel_id][Image2GtDataLayer<Dtype>::Y2];
        ...
        if (do_whaspect) {
          float min_whaspect = this->layer_param_.image_2gt_data_param().min_whaspect();
          float max_whaspect = this->layer_param_.image_2gt_data_param().max_whaspect();
      ...
      for (int ww = 0; ww < windows.size(); ++ww) {
        Dtype x1 = windows[ww][Image2GtDataLayer<Dtype>::X1];
        Dtype y1 = windows[ww][Image2GtDataLayer<Dtype>::Y1];
        Dtype x2 = windows[ww][Image2GtDataLayer<Dtype>::X2];
        Dtype y2 = windows[ww][Image2GtDataLayer<Dtype>::Y2];  
        ...
        if (xc < 0 || xc >= template_width || yc < 0 || yc >= template_height) {
          windows[ww][Image2GtDataLayer<Dtype>::IGNORE] = 1;
        }
        ...
        vector<Dtype> gt(6);
        gt[0] = x1; gt[1] = y1; gt[2] = x2; gt[3] = y2;    ////////////////////////////////gt + gt sub
        gt[4] = windows[ww][Image2GtDataLayer<Dtype>::LABEL];
	gt[5] = windows[ww][Image2GtDataLayer<Dtype>::LABELSUB]; //add
        gts.push_back(gt);
        
        // for gt boxes
        vector<Dtype> gt_box(8);      ////////////////////////////////gt box
        gt_box[0] = dummy; gt_box[1] = x1; gt_box[2] = y1; gt_box[3] = x2; gt_box[4] = y2;
        gt_box[5] = windows[ww][Image2GtDataLayer<Dtype>::LABEL]; 
        gt_box[6] = windows[ww][Image2GtDataLayer<Dtype>::IGNORE];
        gt_box[7] = windows[ww][Image2GtDataLayer<Dtype>::LABELSUB];  // add
      ...
      for (int ww = 0; ww < roni_windows.size(); ++ww) {
        vector<Dtype> roni(4);
        roni[0] = roni_windows[ww][Image2GtDataLayer<Dtype>::X1];
        roni[1] = roni_windows[ww][Image2GtDataLayer<Dtype>::Y1];
        roni[2] = roni_windows[ww][Image2GtDataLayer<Dtype>::X2];
        roni[3] = roni_windows[ww][Image2GtDataLayer<Dtype>::Y2];            
        ...
        for (int h = 0; h < label_height; ++h)
          for (int w = 0; w < label_width; ++w) {
            ...
            for (int ww = 0; ww < ronis.size(); ++ww) {
              ...
              sum_iou += iou;
            }  //\B5\B1ǰλ\D6\C3\D3\EB\CB\F9\D3е\C4dont care IOU֮\BA\CD
            if (sum_iou >= 0.4) {
              top_labels[nn][top_index+5*spatial_dim] = Dtype(1); //\BA\F6\C2\D4
              continue;
            }
            ...
            if (flag && max_iou > fg_threshold) {  //0.5
              float x1 = windows[match_idx][Image2GtDataLayer<Dtype>::X1];
              float y1 = windows[match_idx][Image2GtDataLayer<Dtype>::Y1];
              float x2 = windows[match_idx][Image2GtDataLayer<Dtype>::X2];
              float y2 = windows[match_idx][Image2GtDataLayer<Dtype>::Y2];
              int ignore = windows[match_idx][Image2GtDataLayer<Dtype>::IGNORE];
              if (ignore == 0) { //heavyly occ or trunc 
                  if(ms_sub_label_) {
                       top_labels[nn][top_index] = windows[match_idx][Image2GtDataLayer<Dtype>::LABELSUB];
                  }
                  else {
                       top_labels[nn][top_index] = windows[match_idx][Image2GtDataLayer<Dtype>::LABEL];
                  }
              } else {
      ...
      for (int ww = 0; ww < gts.size(); ++ww) {
        if (windows[ww][Image2GtDataLayer<Dtype>::IGNORE] == 0 && match_times[ww] <= 0){ 
          ...
          const float x1 = windows[ww][Image2GtDataLayer<Dtype>::X1];
          const float y1 = windows[ww][Image2GtDataLayer<Dtype>::Y1];
          const float x2 = windows[ww][Image2GtDataLayer<Dtype>::X2];
          const float y2 = windows[ww][Image2GtDataLayer<Dtype>::Y2];
          ...
          if (ms_sub_label_) {  //labelsub
            top_labels[miss_nn][miss_index] = windows[ww][Image2GtDataLayer<Dtype>::LABELSUB];
          } else {
            top_labels[miss_nn][miss_index] = windows[ww][Image2GtDataLayer<Dtype>::LABEL];
          }
      ...
      inf << image.first << std::endl
          << do_mirror << std::endl;
      for (int ww = 0; ww < windows.size(); ++ww) {
        inf << windows[ww][Image2GtDataLayer<Dtype>::LABEL] <<", "
            << windows[ww][Image2GtDataLayer<Dtype>::IGNORE] <<", "
            << windows[ww][Image2GtDataLayer<Dtype>::X1] <<", "
            << windows[ww][Image2GtDataLayer<Dtype>::Y1] <<", "
            << windows[ww][Image2GtDataLayer<Dtype>::X2] <<", "
            << windows[ww][Image2GtDataLayer<Dtype>::Y2] << std::endl;
      }
      for (int ww = 0; ww < roni_windows.size(); ++ww) {
        inf << 100 << ", "<< 100 << ", "  
            << roni_windows[ww][Image2GtDataLayer<Dtype>::X1] <<", "
            << roni_windows[ww][Image2GtDataLayer<Dtype>::Y1] <<", "
            << roni_windows[ww][Image2GtDataLayer<Dtype>::X2] <<", "
            << roni_windows[ww][Image2GtDataLayer<Dtype>::Y2] << std::endl;
      }
      inf.close();
  ...
  // output gt boxes [img_id, x1, y1, x2, y2, label, ignored] for detection subnet  [img_id, x1, y1, x2, y2, label, ignored, labelsub] 
  if (output_gt_boxes_) {
  ...
  // output gt boxes [img_id, x1, y1, x2, y2, label, ignored, labelsub] 
  if (output_gt_boxes_ && output_2gt_boxes_) {  ////////////////////////////add for the second gtboxes
    int num_gt_boxes = gt_boxes.size();
    const int gt_dim = 7;
    // for special case when there is no gt
    if (num_gt_boxes <= 0) {
      batch->labels_[label_blob_num_+1]->Reshape(1, gt_dim, 1, 1);
      Dtype* gt_boxes2_data = batch->labels_[label_blob_num_+1]->mutable_cpu_data();
      gt_boxes2_data[0]=0; gt_boxes2_data[1]=1; gt_boxes2_data[2]=1; gt_boxes2_data[3]=2;
      gt_boxes2_data[4]=2; gt_boxes2_data[5]=1; gt_boxes2_data[6]=1;
    } else {
      batch->labels_[label_blob_num_+1]->Reshape(num_gt_boxes, gt_dim, 1, 1);
      Dtype* gt_boxes2_data = batch->labels_[label_blob_num_+1]->mutable_cpu_data();
      for (int i = 0; i < num_gt_boxes; i++) {
          gt_boxes2_data[i*gt_dim+0] = gt_boxes[i][0];
		  gt_boxes2_data[i*gt_dim+1] = gt_boxes[i][1];
		  gt_boxes2_data[i*gt_dim+2] = gt_boxes[i][2];
		  gt_boxes2_data[i*gt_dim+3] = gt_boxes[i][3];
		  gt_boxes2_data[i*gt_dim+4] = gt_boxes[i][4];
		  gt_boxes2_data[i*gt_dim+5] = gt_boxes[i][7]; // label for the sub classes
		  gt_boxes2_data[i*gt_dim+6] = gt_boxes[i][6];
      }
    }
  }
...
INSTANTIATE_CLASS(Image2GtDataLayer);
REGISTER_LAYER_CLASS(Image2GtData);
