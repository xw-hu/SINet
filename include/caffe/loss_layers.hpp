...
/////////////////////////////////////////cuhk
template <typename Dtype>
class SmoothL1Loss2Layer : public LossLayer<Dtype> {
 public:
  explicit SmoothL1Loss2Layer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  /**
   * Unlike most loss layers, in the SmoothL1Loss2Layer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
 ...
 protected:
  ...
  bool has_weights_;
  Dtype sigma2_;
  int branch_id_; //branch id (1,2,3...)
};
...
///////////////////////////////////////////////////////////////////////////cuhk hxw
template <typename Dtype>
class SoftmaxWithLoss2Layer : public LossLayer<Dtype> {
 public:
   /**
    * @param param provides LossParameter loss_param, with options:
    *  - ignore_label (optional)
    ...
    *    present; otherwise the loss is simply summed over spatial locations.
    */
  explicit SoftmaxWithLoss2Layer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "SoftmaxWithLoss2"; }
  virtual inline int ExactNumBottomBlobs() const { return 3; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline int MaxTopBlobs() const { return 2; }
 ...
 protected:
  ...
  int softmax_axis_, outer_num_, inner_num_;
  //int num_true_roi_; //ingore the padding roi
  int branch_id_; //branch id (1,2,3...)
};
