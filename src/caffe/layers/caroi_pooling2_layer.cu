...
__global__ void CAROIPool2Forward(const int nthreads, const Dtype* bottom_data,
    const Dtype spatial_scale, const int channels, const int height,
    const int width, const int pooled_height, const int pooled_width,
    const Dtype pad_ratio_normal, const Dtype pad_ratio_small, const Dtype* bottom_rois, Dtype* top_data, Dtype* top_data_small, int* argmax_data, const Dtype* bottom_hash, bool normal_branch, bool small_branch) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (bottom_hash[1]==2 && !small_branch) //small
    {return;}
    if (bottom_hash[1]==1 && !normal_branch) //normal
    {return;}

    bottom_rois += n * 5;
    int roi_batch_ind = bottom_rois[0];
    //printf("roi_batch_ind: %d\n",roi_batch_ind);

    // padding
    Dtype pad_w, pad_h, pad_ratio;

    if (bottom_hash[1]==2) //small
    {
        pad_ratio = pad_ratio_small;
    }
    else //if(bottom_hash[1]==1)
    {
        pad_ratio = pad_ratio_normal;
    }
    ...
    if (bin_size_h_float<1 || bin_size_w_float<1)
    {
       if (bin_size_h_float>1)  //height is large
       {
           ...
           if (bottom_hash[1]==2) //small
           {
               top_data_small[((new_n*channels + c)*pooled_height + ph) * pooled_width + pw] = maxval;
           }
           else //if(bottom_hash[1]==1)
           {
               top_data[((new_n*channels + c)*pooled_height + ph) * pooled_width + pw] = maxval;
           }
       ...
       else
       {
           ...
           if (bottom_hash[1]==2) //small
           {
               top_data_small[((new_n*channels + c)*pooled_height + ph) * pooled_width + pw] = static_cast<Dtype>(b * result_temp2 + (-result_temp1 * b + result_temp1));
           }
           else
           {
               top_data[((new_n*channels + c)*pooled_height + ph) * pooled_width + pw] = static_cast<Dtype>(b * result_temp2 + (-result_temp1 * b + result_temp1));
           }
...
template <typename Dtype>
void CAROIPooling2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int count_suffix = top[0]->count(1); //chw
  int count_roi = bottom[1]->shape(0); //n
  int count = count_suffix * count_roi;

  Dtype *top_data, *top_data_small; 
  if (normal_branch_ && small_branch_)
  {
     top_data = top[0]->mutable_gpu_data();
     top_data_small = top[1]->mutable_cpu_data();
  }
  else if (normal_branch_) //!small
  {
     top_data = top[0]->mutable_gpu_data();
     if(bottom[1]->num()-small_roi_ == 0)
     {return;}
  }
  else if (small_branch_) //!normal
  {
     top_data_small = top[0]->mutable_cpu_data();
     if(small_roi_ == 0)
     {return;}
  }
  ...
  CAROIPool2Forward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count , bottom_data, spatial_scale_, channels_, height_, width_,
      pooled_height_, pooled_width_, pad_ratio_normal_, pad_ratio_small_, bottom_rois, top_data, top_data_small, argmax_data, bottom_hash, normal_branch_, small_branch_);
  CUDA_POST_KERNEL_CHECK;
}

template <typename Dtype>
__global__ void CAROIPool2Backward(const int nthreads, const Dtype* top_diff, const Dtype* top_diff_small,
    const int* argmax_data, const int num_rois, const Dtype spatial_scale,
    const int channels, const int height, const int width,
    const int pooled_height, const int pooled_width, const Dtype pad_ratio_normal, const Dtype pad_ratio_small, Dtype* bottom_diff, const Dtype* bottom_rois, const Dtype* bottom_hash, bool normal_branch, bool small_branch) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    //printf("Backward!!!!");
    ...
    int n = index / width / height / channels;  //batch n
    ...
    for (int roi_n = 0; roi_n < num_rois; ++roi_n) {
      ...
      if (bottom_hash[roi_n*2+1]==2 && !small_branch) //small
      {continue;}
      if (bottom_hash[roi_n*2+1]==1 && !normal_branch) //normal
      {continue;}

      // padding
      Dtype pad_w, pad_h, pad_ratio;

      if (bottom_hash[roi_n*2+1]==2) //small
      {
          pad_ratio = pad_ratio_small;
      }
      else //if(bottom_hash[roi_n*2+1]==1)
      {
          pad_ratio = pad_ratio_normal;
      }
      ...
      int new_n = ((bottom_hash[roi_n*2] <0 )?0:bottom_hash[roi_n*2]);
      ...
      if (bin_size_h_float<1 || bin_size_w_float<1)
      {
         const Dtype* offset_top_diff;
         if (bottom_hash[roi_n*2+1] == 2) //small
         {
            offset_top_diff = top_diff_small + ((new_n * channels + c) * pooled_height * pooled_width);
         }
         else
         {
            offset_top_diff = top_diff + ((new_n * channels + c) * pooled_height * pooled_width);
         }
         if (bin_size_h_float>1)  //height is large
         { 
             Dtype bin_size_h = static_cast<Dtype>(bin_size_h_float);
      ...
      const Dtype* offset_top_diff_normal;
      
      if (bottom_hash[roi_n*2+1] == 1) //normal
      {
          offset_top_diff_normal = top_diff + ((new_n * channels + c) * pooled_height * pooled_width);
      }
      else
      {
          offset_top_diff_normal = top_diff_small + ((new_n * channels + c) * pooled_height * pooled_width);
      }
      ...
      for (int ph = phstart; ph < phend; ++ph) {
        for (int pw = pwstart; pw < pwend; ++pw) {
          if (offset_argmax_data[ph * pooled_width + pw] == (h * width + w)) {
            gradient += offset_top_diff_normal[ph * pooled_width + pw];
          }
        }
      }
    }
    //printf("index: %d     gradient: %lf\n", index, gradient);
    bottom_diff[index] = gradient;
  }
}

template <typename Dtype>
void CAROIPooling2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  const Dtype* top_diff, *top_diff_small;
  if (normal_branch_ && small_branch_)
  {
     top_diff = top[0]->gpu_diff();
     top_diff_small = top[1]->gpu_diff();
  }
  else if (normal_branch_) //!small
  {
     top_diff = top[0]->gpu_diff();
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
  CAROIPool2Backward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, top_diff, top_diff_small, argmax_data, bottom[1]->num(), spatial_scale_, channels_, //bottom[1]->num() -- top[0]->num
      height_, width_, pooled_height_, pooled_width_, pad_ratio_normal_, pad_ratio_small_, bottom_diff, bottom_rois, bottom_hash, normal_branch_, small_branch_);
  CUDA_POST_KERNEL_CHECK;
}

INSTANTIATE_LAYER_GPU_FUNCS(CAROIPooling2Layer);
